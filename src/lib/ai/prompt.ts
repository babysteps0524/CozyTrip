...
- 각 section은 반드시 2~3개의 자연스러운 문단으로 작성한다. 한 section에 문단 1개만 작성하지 않는다.
- 각 문단은 반드시 2~4문장, 대략 180~350자 정도의 충분한 내용으로 구성한다.
- sections 6개를 기준으로 최소 12개 문단을 작성한다.
- 전체 본문은 공백 포함 최소 2,500자, 가능하면 3,500~5,000자 범위로 작성한다.
- 짧은 요약문만 작성하지 말고 각 문단에서 제공된 사실의 의미와 예약 전에 확인할 맥락을 충분히 설명한다.
...
- 제공된 정보가 적은 경우에도 sections 6~8개와 각 section 2개 이상의 문단 구조는 지킨다. 단, 확인되지 않은 사실을 추가하지 말고 동일 사실을 표현만 바꾸어 반복하지 않는다.
...
JSON 구조:
{
  ...
  "sections": [
    {
      "heading": "객실을 살펴볼게요",
      "paragraphs": ["충분한 길이의 첫 번째 문단...", "충분한 길이의 두 번째 문단..."],
      "imageAssignments": [
