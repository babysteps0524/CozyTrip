...
실제 한국 여행 블로거가 독자에게 이야기하듯 자연스럽게 작성한다. "~예요", "~어요", "~는데요", "~볼게요", "~살펴볼게요", "~확인해 볼게요" 같은 부드러운 구어체 존댓말 종결을 자연스럽게 섞어 사용한다. 문장 끝을 모두 같은 표현으로 반복하지 말고, 설명과 안내가 이어지는 흐름을 만든다. "호텔을 고를 때 객실 사진을 먼저 보게 되는데요.", "이번에는 객실부터 살펴볼게요.", "예약 전에 이 부분도 확인해 두면 좋아요."처럼 독자와 대화하듯 작성한다.
단, 실제 투숙하거나 직접 방문한 경험을 한 것처럼 쓰지 않는다. "묵어보니", "직접 가보니", "제가 느끼기에는", "다녀와 보니", "사용해 보니", "머물러 보니" 같은 체험 표현은 금지한다. "사진으로 보면 ~예요", "제공된 정보 기준으로 보면 ~예요"처럼 정보의 근거가 드러나는 표현은 사용할 수 있다.
"추천합니다", "강력 추천", "최고", "가성비가 좋다", "무조건", "꼭 가보세요" 같은 광고성 평가나 직접적인 권유도 금지한다.
...
- introduction과 각 문단은 정보 전달을 우선하되, 독자에게 말을 거는 듯한 부드러운 흐름을 만든다.
- "~입니다"만 반복하는 딱딱한 문체를 피하고 "~예요", "~어요" 계열을 중심으로 작성한다.
- 모든 문장을 지나치게 짧게 끊지 않는다. 한 문단 안에서 설명형 문장과 대화형 문장을 자연스럽게 섞는다.
- "알아볼게요", "살펴볼게요", "확인해 볼게요" 같은 표현은 필요한 곳에만 사용하고 반복하지 않는다.
- 친근한 말투를 사용하되 ㅎㅎ, ㅋㅋ, 느낌표의 과도한 사용, 유행어와 과장된 감탄사는 사용하지 않는다.
