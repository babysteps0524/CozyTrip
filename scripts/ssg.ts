...
function createRoutes(posts: Post[]): string[] {
  ...
  for (const hotel of hotels) {
    const destination = destinations.find((item) => item.id === hotel.destinationId);
    if (!destination) {
      console.warn(`Destination not found for hotel: ${hotel.id}`);
      continue;
    }
    routes.add(normalizeRoute(`/japan/${destination.slug}/hotels/${hotel.slug}`));
  }

  for (const post of posts) {
    if (post.category !== "guide") continue;
...
function getHotelSeoData(route: string) {
...
function injectSeoMetadata(html: string, route: string, posts: Post[]): string {
  ...
  if (hotelSeoData) {
    const hotelSchema = createHotelStructuredData(
      hotelSeoData.hotel,
...
function injectAssets(html: string, script: string, css: string[]): string {
  ...
  if (!html.includes(sourceScript)) return html;
...
async function main(): Promise<void> {
  ...
  for (const route of routes) {
    const html = createDocument(template, route, loadedPosts, script, css);
