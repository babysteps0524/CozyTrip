...
async function main(): Promise<void> {
  ...
  const plannedInventory = report.destinations.reduce(
  ...
  if (plannedInventory > report.totalInventory) {
    fail("Planned destination inventory cannot exceed total inventory.");
  }
  assertEqual(
    "totalPublished",
  ...
  assertEqual(
    ...
    Math.max(0, hotelFile.hotels.length - report.totalPublished),
  );
